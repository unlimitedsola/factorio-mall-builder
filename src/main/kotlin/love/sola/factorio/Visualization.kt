...
import love.sola.factorio.data.Ingredient
...
fun genDAG(recipes: List<Recipe>): Image {
    var graph = Factory.graph("Visualization")
        .directed().strict()
    val nodes = recipes.flatMap { it.ingredients.map(Ingredient::name) + it.products.map(Product::name) }.distinct()
        .associateWithTo(hashMapOf()) { Factory.node(it).with(Shape.RECTANGLE) }
    for (recipe in recipes) {
        recipe.ingredients.forEach { ingredient ->
            recipe.products.forEach { product ->
                val parentRecipe = recipes.find { r -> r.products.any { p -> p.name == ingredient.name } }
                if (parentRecipe == null) {
                    nodes[ingredient.name] = nodes[ingredient.name]!!.link(
